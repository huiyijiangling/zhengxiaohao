...
checkGPL(gpl)
printGPLInfo(gpl)
probe2gene=idmap(gpl=gpl,type = "soft")#gpl570 用soft更好，soft的合并性差落后，建议首选bioc,但如果结合了更新就不一样了,例如ta
head(probe2gene)
# 发现有可能是一对多
...
phenoDat$tn=ifelse(phenoDat$`tissue:ch1`=='gastric cancer tumor',"T","N")
...
if(T){
  ...
  GSE54129_5050=TumorOnlysplitinto5050(genes_expr,"ENSG00000113504",phenoDat,"geo_accession","tn","T")
  DEGAll_GSE54129_5050 <- Limma_microarray_rma(eset     = GSE54129_5050[[1]][[2]], 
                                               group      = GSE54129_5050[[1]][[1]]$grouphl, 
                                               comparison = 'H-L')
  
}

save(DEGAll_GSE54129,DEGAll_GSE54129_5050,file ="DEGAll_GSE54129_filter.Rdata")
write.csv(DEGAll_GSE54129,quote = T,file = "DEGAll_GSE54129.csv")
write.csv(DEGAll_GSE54129_5050,quote = T,file = "DEGAll_GSE54129_5050.csv")
write.csv(GSE54129_5050[[1]][[2]],quote = T,file = "expr_GSE54129_5050.csv")
write.csv(GSE54129_5050[[1]][[1]],quote = T,file = "ph_GSE54129_5050.csv")
# save(DEGAll_GSE54129,file ="DEGAll_GSE54129.Rdata" )

# load("DEGAll_GSE54129.Rdata")
