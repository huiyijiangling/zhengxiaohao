...
# kkk=log2(kkk+1)
# boxplot(kkk,las=2)
# gset=AnnoProbe::geoChina('GSE60646')
# gset=getGEO('GSE60646',getGPL = F)
# save(gset,file="GSE60646_eSet.Rdata")
load("./GSE60646_eSet.Rdata")#log不平，但像是弄过了
...
phenoDat <- pData(eSet)
head(phenoDat[,1:4])
phenoDat$patient_id=stringr::str_split(phenoDat$supplementary_file,"_AX",simplify = T)[,2]
phenoDat$patient_id=stringr::str_split(phenoDat$patient_id,"\\.",simplify = T)[,1]
phenoDat$patient_id=paste0("AX",phenoDat$patient_id)
table(phenoDat$patient_id)
write.csv(phenoDat,file="phenoDat_GSE60646.csv",quote = T)
#AX0001 AX0235
...
GPL[GPL==""] <- NA
table(GPL[!is.na(GPL$gene_assignment),])
GPL=subset(GPL,gene_assignment !="---")
GPL$symbol=stringr::str_split(GPL$gene_assignment," // ",n = 3,simplify = T)[,2]
probe2gene=unique(GPL[,c("ID","symbol")])
colnames(probe2gene)=c("probe_id","symbol")
...
phenoDat_2=subset(phenoDat,phenoDat$source_name_ch1!="PDAC_untreated")
genes_expr_2=subset(genes_expr,select=phenoDat_2$geo_accession)
phenoDat_2$group=ifelse(phenoDat_2$`treatment:ch1`=="neoadjuvant chemoradiation","neo","none")
DEGAll <- Limma_microarray_rma(eset     = genes_expr_2, 
                                             group      = phenoDat_2$group, 
                                             comparison = "neo-none")
DEGAll$FDR=as.numeric(DEGAll$FDR)
DEGAll=subset(DEGAll,DEGAll$FDR<0.05)
DEGAll=subset(DEGAll,DEGAll$logFC>0)
DEGAll$X=rownames(DEGAll)
openxlsx::write.xlsx(DEGAll,file = "GEMpluserlotinib_vs_naive_paad_pos.Rdata")
gem1=read.csv(r"(C:\Users\zxh\Desktop\R\paad-tcga-gtex\耐药分为1年内和1年外1875.csv)")
gem1=gem1[gem1$PValue<0.05,]
gem2=read.csv(r"(C:\Users\zxh\Desktop\R\paad-tcga-gtex\耐药分为1年内和2年外1825.csv)")
gem2=gem2[gem2$PValue<0.05,]
rownames(DEGAll)
DEGAll_1=merge(DEGAll,gem1,by="X")
openxlsx::write.xlsx(DEGAll_1,file = "GEMpluserlotinib_vs_naive_paad_pos_genefrom耐药分为1年内和1年外1875.xlsx")
DEGAll_2=merge(DEGAll,gem2,by="X")
openxlsx::write.xlsx(DEGAll_2,file = "GEMpluserlotinib_vs_naive_paad_pos_genefrom耐药分为1年内和2年外1825.xlsx")
intersect(DEGAll_1$X,DEGAll_2$X)
share=DEGAll_1[DEGAll_1$X%in%intersect(DEGAll_1$X,DEGAll_2$X),]
share=merge(share,gem2,by="X")
openxlsx::write.xlsx(share,file = "GEMpluserlotinib_vs_naive_paad_pos_genefrom_overlap38.xlsx")
#check
boxplot(genes_expr_2["ENSG00000005884",],phenoDat_2$group)

load(file="dakala.Rdata")
# # 修剪+median !!!!!!!!!!!!!!!!!!!!!!!!
# dim(phenoDat)
