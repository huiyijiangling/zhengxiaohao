...
# kkk=log2(kkk+1)
# boxplot(kkk,las=2)
#gset=AnnoProbe::geoChina('GSE29272')
load("./GSE29272_eSet.Rdata")#log不平，但像是弄过了
...
system("tar -xvf ./GSE29272/GSE29272_RAW.tar -C ./GSE29272/")
celFiles <- list.celfiles('./GSE29272/',full.name=TRUE,listGzipped = T)
...
#
#read_xls处理预后信息
GSE29272_outcome=read.csv("./GSE29272/GSE29272_outcome.csv",header = T)
GSE29272_outcome=as.data.frame(GSE29272_outcome)
#
pdf("GSE29272_boxplot.pdf",width=100)
par(mfrow = c(2,1));
boxplot(probes_expr,las=2)
...
# phenoDat=merge(phenoDat,GSE29272_outcome,by.x="patient:ch1",by.y="Tumor.ID")
...
phenoDat$tn=substr(phenoDat$patientidtn,6,6)
# phenoDat$tn=ifelse(phenoDat$`tissue:ch1`=='Gastric tumor',"T","N")
DEGAll_GSE29272 <- Limma_microarray_paired(eset     = genes_expr, 
                                        group      = phenoDat$tn, 
                                        paired     =phenoDat$patientid,
                                        comparison = 'T-N')
save(DEGAll_GSE29272,file ="DEGAll_GSE29272.Rdata" )
# 修剪+median !!!!!!!!!!!!!!!!!!!!!!!!
dim(phenoDat)
...
phenoDat=merge(phenoDat,GSE29272_outcome,by.x="name",by.y="ID")
rownames(phenoDat)=phenoDat$geo_accession
...
#
library(DescTools)
genes_expr_mad_GSE29272=t(RobScale(t(genes_expr_with_clinical)))
genes_expr_mean_GSE29272=t(scale(t(genes_expr_with_clinical)))
probes_expr_GSE29272=probes_expr
phenoDat_GSE29272=phenoDat
save(genes_expr_mad_GSE29272,genes_expr_mean_GSE29272,probes_expr_GSE29272,phenoDat_GSE29272,file="GSE29272_after_bioc.Rdata")

pdf("GSE29272_mad.pdf",width = 100)
par(mfrow = c(2,1));
