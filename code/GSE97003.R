...
# kkk=log2(kkk+1)
# boxplot(kkk,las=2)
#gset=AnnoProbe::geoChina('GSE97003')
# gset=getGEO('GSE97003',getGPL = F)
# save(gset,file = "GSE97003_eSet.Rdata")
load("./GSE97003_eSet.Rdata")#log不平，但像是弄过了
...
if(T){
  # gset=getGEO('GSE97003',getGPL = F)
  # getGEOSuppFiles('GSE97003')
  
  #windows powershell
  # 更改文件权限
  system("tar -xvf ./GSE97003/GSE97003_RAW.tar -C ./GSE97003/")
  
  #或者        filenames <- file.path(path, metadata$file_id, metadata$file_name, fsep = .Platform$file.sep)
  filenames_R <- list.files('./GSE97003/',pattern = "noStrand.counts.txt.gz",full.name=TRUE)
  ...
  probes_expr_GSE97003_rna=rnaCount
  phenoDat_GSE97003=phenoDat
  save(probes_expr_GSE97003_rna,phenoDat_GSE97003,file ="GSE97003_before.Rdata" )
  write.csv(phenoDat_GSE97003,file="phenoDat_GSE97003.csv",quote = T)
  #https://ftp.ncbi.nlm.nih.gov/geo/series/GSE119nnn/GSE97003/matrix/GSE97003_series_matrix.txt.gz
  #https://ftp.ncbi.nlm.nih.gov/geo/series/GSE119nnn/GSE97003/matrix
  #https://ftp.ncbi.nlm.nih.gov/geo/series/GSE119nnn/GSE97003/suppl/ 如果没上传就放在这
  #有时候原来的矩阵是没有上传的 那就可以去getGEO   
  #getGEOfile('GSE97003') 这个没有用
}
if(eSet@annotation=="GPL570"){
  system("tar -xvf ./GSE97003/GSE97003_RAW.tar -C ./GSE97003/")
  celFiles <- list.celfiles('./GSE97003/',full.name=TRUE,listGzipped = T)
  ...
  dim(probes_expr_rma)
  #
  pdf("GSE97003_boxplot.pdf",width=100)
  par(mfrow = c(2,1));
  boxplot(probes_expr,las=2)
...
#
#read_xls处理预后信息
# phenoDat_outcome=read.csv("./GSE97003/GSE97003_outcome.csv",header = T)
...
load("GSE97003_before.Rdata")
probe2gene=data.frame(probe_id=rownames(probes_expr_GSE97003_rna),symbol=rownames(probes_expr_GSE97003_rna))
source("./updateName.R")
...
probes_expr=probes_expr_GSE97003_rna
genes_expr <- filterEM(probes_expr,probe2gene)
genes_expr_GSE97003_rna=genes_expr
# probes_expr_GSE97003_rna["MIR4435-1HG",]
...
#只好自己造了
GPL=data.frame(rownames(probes_expr_GSE97003_mir),rownames(probes_expr_GSE97003_mir))
colnames(GPL)=c("ID","miRNA_ID")
miRNANames=rownames(probes_expr_GSE97003_mir)
...
probes_expr=probes_expr_GSE97003_mir
# printGPLInfo(gpl)
# probe2gene=idmap(gpl=gpl,type = "bioc")#soft的合并性差落后，建议首选bioc
# head(probe2gene)
genes_expr <- filterEM(probes_expr,probe2gene)
genes_expr_GSE97003_mir=genes_expr
# head(genes_expr)
# 修剪+median !!!!!!!!!!!!!!!!!!!!!!!!
# dim(phenoDat)
...
save(probes_expr_GSE97003_mir,genes_expr_GSE97003_mir,
     probes_expr_GSE97003_rna,genes_expr_GSE97003_rna,
     phenoDat_GSE97003,file ="GSE97003_before.Rdata")
...
if(F){
  library(DescTools)
  #
  genes_expr_mad_GSE97003_rna=t(RobScale(t(rnaCount)))
  genes_expr_mean_GSE97003_rna=t(scale(t(rnaCount)))
  
  
  genes_expr_mad_GSE97003_mir=t(RobScale(t(mirCount)))
  genes_expr_mean_GSE97003_mir=t(scale(t(mirCount)))
  
  
  
  save(genes_expr_mad_GSE97003,genes_expr_mean_GSE97003,probes_expr_GSE97003,phenoDat_GSE97003,file="GSE97003_after_bioc.Rdata")
  write.csv(phenoDat_GSE97003,file="phenoDat_GSE97003.csv",quote = T)
  pdf("GSE97003_mad.pdf",width = 100)
  par(mfrow = c(2,1));
