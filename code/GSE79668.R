...
# kkk=log2(kkk+1)
# boxplot(kkk,las=2)
#gset=AnnoProbe::geoChina('GSE79668')
#GSE79668没有上传矩阵
#GSE79668没有只上传了解释
#发现是腺癌，没说一定是ductal
gset <- getGEO(filename = "GSE79668_series_matrix.txt.gz")
phenoDat <- pData(eSet)


# load("./GSE79668_eSet.Rdata")#log不平，但像是弄过了
...
if(eSet@annotation=="GPL570"){
  system("tar -xvf ./GSE79668/GSE79668_RAW.tar -C ./GSE79668/")
  celFiles <- list.celfiles('./GSE79668/',full.name=TRUE,listGzipped = T)
  ...
  dim(probes_expr_rma)
  #
  pdf("GSE79668_boxplot.pdf",width=100)
  par(mfrow = c(2,1));
  boxplot(probes_expr,las=2)
...
#
#read_xls处理预后信息
phenoDat_outcome=read.csv("./GSE79668/GSE79668_outcome.csv",header = T)
...
#
library(DescTools)
genes_expr_mad_GSE79668=t(RobScale(t(genes_expr_with_clinical)))
genes_expr_mean_GSE79668=t(scale(t(genes_expr_with_clinical)))
probes_expr_GSE79668=probes_expr
phenoDat_GSE79668=phenoDat
save(genes_expr_mad_GSE79668,genes_expr_mean_GSE79668,probes_expr_GSE79668,phenoDat_GSE79668,file="GSE79668_after_bioc.Rdata")
write.csv(phenoDat_GSE79668,file="phenoDat_GSE79668.csv",quote = T)
pdf("GSE79668_mad.pdf",width = 100)
par(mfrow = c(2,1));
