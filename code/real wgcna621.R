...
load("./rnatpm_stomach_WGCNA.Rdata")
...
dat=rnatpmlog2001_stomach[which(rownames(rnatpmlog2001_stomach) %in% rownames(rnatpm_stomach_mrna)),]#dat=rnatpm_stomach_mrna
# 每次都要检测数据
dat[1:4,1:4] 
dim(dat)
# 这里直接删除离群 只有一个离群值时高高挂起，两个就是单独垂下，其实为了好看可以吧plot(,hang=-1) cex=1, 调整间距 ,leaflab="none" 不显示标签
dat=subset(dat,select=c(-`GTEX-U8XE-2126-SM-5CHRC`,-`GTEX-WFON-1126-SM-5CHTG`,-`TCGA-BR-6710-01`,-`TCGA-SW-A7EB-01`))
TcgaTargetGTEX_phenotype_stomach=TcgaTargetGTEX_phenotype_stomach[-which(rownames(TcgaTargetGTEX_phenotype_stomach) %in% c("GTEX-U8XE-2126-SM-5CHRC","GTEX-WFON-1126-SM-5CHTG","TCGA-BR-6710-01","TCGA-SW-A7EB-01")),]
#
group_list=ifelse(TcgaTargetGTEX_phenotype_stomach$sample_type=="PrimaryTumor",'tumor','normal')
...
if(T){
  ...
  WGCNA_matrix = t(RNAseq_voom[order(apply(RNAseq_voom,1,mad), decreasing = T)[1:9000],])
...
if(T){
  powers = c(1:15)#c(c(1:10), seq(from = 12, to=20, by=2))
