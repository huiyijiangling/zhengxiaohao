...
MIR31HGtxt=read.csv(file="MIR31HG.csv",skip = 0)
colnames(CDKN2Atxt)
...
box=box[,-(1:7)]
write.csv(box,file="co-CDKN2A-CDKN2B-MTAP-count.csv",quote=T)
box=TcgaTargetGTEX_phenotype_Pancreas624
# box$cnv=ifelse(TcgaTargetGTEX_phenotype_Pancreas624$cnv=="PrimaryTumor",'tumor','normal')
box=cbind(box,t(rnaCountslog21_Pancreas["ENSG00000171889",]))
box=box[,-(1:7)]
write.csv(box,file="co-CDKN2A-CDKN2B-MTAP-log21-count.csv",quote=T)
#
box=TcgaTargetGTEX_phenotype_Pancreas624
# box$cnv=ifelse(TcgaTargetGTEX_phenotype_Pancreas624$cnv=="PrimaryTumor",'tumor','normal')
box=cbind(box,t(rnatpmlog2001_Pancreas["ENSG00000171889",]))
box=box[,-(1:7)]
write.csv(box,file="co-CDKN2A-CDKN2B-MTAP-log2001-tpm.csv",quote=T)

box=TcgaTargetGTEX_phenotype_Pancreas624
# box$cnv=ifelse(TcgaTargetGTEX_phenotype_Pancreas624$cnv=="PrimaryTumor",'tumor','normal')
box=cbind(box,t(rnatpm_Pancreas["ENSG00000171889",]))
box=box[,-(1:7)]
write.csv(box,file="co-CDKN2A-CDKN2B-MTAP-tpm.csv",quote=T)
# pdf('Figure A.Boxplot of codelete.pdf',width = 8,height=4)
# if(require('ggpubr')){
#   library(ggpubr)
...
# pdf('Figure A.Boxplot of MLLT3 baed on status of codelete.pdf',width = 8,height=4)
# if(require('ggpubr')){
#   library(ggpubr)
#   # google search : ggpubr boxplot add p-value
#   # http://www.sthda.com/english/articles/24-ggpubr-publication-ready-plots/76-add-p-values-and-significance-levels-to-ggplots/
...
# bartlett.test(weight~group,data=my_data)#方差齐性
pdf('Figure A.Boxplot of MIR31HG baed on status of codelete count log gtex.pdf',width = 8,height=4)
if(require('ggpubr')){
  library(ggpubr)
  # google search : ggpubr boxplot add p-value
  # http://www.sthda.com/english/articles/24-ggpubr-publication-ready-plots/76-add-p-values-and-significance-levels-to-ggplots/
...
dev.off()
?stat_compare_means
write.csv(codelete,"codelete.csv")
