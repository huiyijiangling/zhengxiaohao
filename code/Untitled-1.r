...
# dat=rnatpmlog2001_Pancreas[which(rownames(rnatpmlog2001_Pancreas) %in% rownames(rnatpm_Pancreas_mrna)),]#dat=rnatpm_Pancreas_mrna
...
# TcgaTargetGTEX_phenotype_Pancreas=TcgaTargetGTEX_phenotype_Pancreas[-which(rownames(TcgaTargetGTEX_phenotype_Pancreas) %in% c("GTEX-U8XE-2126-SM-5CHRC","GTEX-WFON-1126-SM-5CHTG")),]
#
group_list=ifelse(TcgaTargetGTEX_phenotype_Pancreas624$sample_type=="PrimaryTumor",'tumor','normal')
...
if(T){
  ...
  WGCNA_matrix = t(RNAseq_voom[order(apply(RNAseq_voom,1,mad), decreasing = T)[1:12000],])
...
if(T){
  ...
  sft = pickSoftThreshold(datExpr, powerVector = powers, RsquaredCut=0.900, verbose = 5)
