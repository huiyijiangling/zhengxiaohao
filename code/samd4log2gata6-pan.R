...
gc()
# library(GDCRNATools)
#注意pan-cancer时log是失效的
SMAD4txt=read.table(file="C:/Users/zxh/Desktop/R/paad-tcga-gtex/hic scripts/samd_gata6/PAN-SMAD4-GATA6.txt",sep = "\t",header = T)
colnames(SMAD4txt)
codelete=SMAD4txt
...
box=box[,-(1:7)]
write.csv(box,file="PAN-SMAD4-GATA6.csv",quote=T)
...
pdf('Figure A.Boxplot of MIR31HG baed on status of codelete samd4log2gata6-pan.pdf',width = 8,height=8)
if(require('ggpubr')){
  library(ggpubr)
  ...
  p <- ggboxplot(box, x = "GATA6status", y = "number1",fill="cnv3",
    ...
    labs(title="",x="GATA6", y = "Expression")#+#tag="ENSG00000000003"

  # ?guides
  # geom_dotplot(binaxis='y', stackdir='center', stackratio=1, dotsize=0.2,fill = c("#999999"))
  #  Add p-value
  p + stat_compare_means(aes(group=cnv3),method = "wilcox.test", label.y = 20 )
