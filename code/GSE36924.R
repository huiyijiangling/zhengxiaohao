#GSE36924 没修改，其实是paca-au但是根本没有对应
rm(list=ls())
...
# kkk=log2(kkk+1)
# boxplot(kkk,las=2)
#gset=AnnoProbe::geoChina('GSE36924')
load("./GSE36924_eSet.Rdata")#log不平，但像是弄过了
...
if(eSet@annotation=="GPL570"){
  system("tar -xvf ./GSE36924/GSE36924_RAW.tar -C ./GSE36924/")
  celFiles <- list.celfiles('./GSE36924/',full.name=TRUE,listGzipped = T)
  ...
  dim(probes_expr_rma)
  #
  pdf("GSE36924_boxplot.pdf",width=100)
  par(mfrow = c(2,1));
  boxplot(probes_expr,las=2)
...
#
#read_xls处理预后信息
GSE36924_outcome=read.csv("./GSE36924/GSE36924_outcome.csv",header = T)
GSE36924_outcome=as.data.frame(GSE36924_outcome)
#
phenoDat=merge(phenoDat,GSE36924_outcome,by.x="patient:ch1",by.y="Tumor.ID")
...
#
library(DescTools)
genes_expr_mad_GSE36924=t(RobScale(t(genes_expr_with_clinical)))
genes_expr_mean_GSE36924=t(scale(t(genes_expr_with_clinical)))
probes_expr_GSE36924=probes_expr
phenoDat_GSE36924=phenoDat
save(genes_expr_mad_GSE36924,genes_expr_mean_GSE36924,probes_expr_GSE36924,phenoDat_GSE36924,file="GSE36924_after_bioc.Rdata")
write.csv(phenoDat_GSE36924,file="phenoDat_GSE36924.csv",quote = T)
pdf("GSE36924_mad.pdf",width = 100)
par(mfrow = c(2,1));
